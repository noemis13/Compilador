...
#				self.builder.load(resExp)

				self.builder.store(resExp, self.tabelaSimbolos[self.verifica_escopo(var)][1])
		...
		if expRetorna in self.valoresExpressoes: 
			...
			if self.valoresExpressoes[expRetorna][2] == "+":
				if tipo == "inteiro" and self.valoresExpressoes[expRetorna][1] == "inteiro":
					...
					somaStore = self.builder.store(soma, self.tabelaSimbolos[escopoExp][1])
...
#			nomeEscopo = "global-"+expRetorna
	...
	def leia(self, node):
		valor = self.builder.call(self.leiaInteiro, [], "leiaInteiro")
		print(valor)
		...
		if escopoNode in self.tabelaSimbolos:
			...
			if tipoNode == "inteiro":
				return self.builder.store(self.float_to_int(valor), self.tabelaSimbolos[escopoNode][1])
				#return self.builder.load(self.tabelaSimbolos[escopoNode][1])
			elif tipoNode == "flutuante":
				return self.builder.store(valor, self.tabelaSimbolos[escopoNode][1])
				#return self.builder.load(self.tabelaSimbolos[escopoNode][1])
	...
	def escreva(self, node):
		...
		if tipo == "inteiro":
			valorExp = (self.builder.load(self.tabelaSimbolos[escopoValor][1]))
			return self.escrevaInteiro(valorExp)
			
			#valorExp = self.int_to_float(self.builder.load(self.tabelaSimbolos[escopoValor][1]))
			#return self.builder.call(self.escrevaInteiro, [valorExp])
			...
			return self.escrevaFlutuante(valorExp)
