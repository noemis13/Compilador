...
\title{Compiladores: análise léxica, sintática e semântica}
...
\begin{abstract}
 This meta-article describes the procedures and results of the development of a lexical, syntactic and semantic analyzer. The first one is responsible for verifying the input of character lines from a source code written T++, producing a sequence of lexical symbols. The second analyzes the source code in T++ and returns an abstract tree. The third runs through this tree in order to find and report errors in the language. For the creation of these analyzers, a Python language library called PLY and YACC was used.
 
\end{abstract}
     
\begin{resumo} 
  Este meta-artigo descreve os procedimentos e resultados do desenvolvimento de um analisador léxico, sintático e semântico. O primeiro é responsável por verificar as entradas de linhas de caracteres de um código fonte escrito T++, produzindo uma sequência de símbolos léxicos. O segundo analisa o código fonte em T++ e retorna uma árvore abstrata. Por fim, o terceiro percorre essa árvore com o objetivo de encontrar e informar erros na linguagem. Para a criação desses analisares, foi utilizado uma biblioteca da linguagem Python denominado PLY e YACC.
...
\subsection{Objetivo}
O objetivo desse trabalho é desenvolver um programa capaz de realizar a análise léxica, sintática e semântica de códigos fontes escrito na Linguagem T++, retornando uma arvore sintática abstrata e erros presentes na linguagem.
...
Para verificar a existências de erros, foi analisado cada nó filho (saída da árvore), percorrendo suas instruções até chegar em alguma instrução no código, como declaração e inicialização de variáveis ou chamadas de funções, verificando a existência de alguma inconsistência não condizente que a estrutura permitida na linguagem. A medida que essas instruções são percorridas, ao identificar variáveis ou funções, elas são adicionadas em uma tabela de símbolos, armazenando seus valores que auxiliam a definir se a ação é ou não permitida no código.
...
\texttt{\{'\textbf{principal}': ['funcao', 'principal', 'void', False, 'inteiro', 0],}

\texttt{'\textbf{global-b}': ['variavel', 'b', True, True, 'inteiro', 0],}\}

Para as funções, como a \texttt{principal}, na primeira posição do vetor é armazenado o atributo (função), na segunda o nome da função, na terceira armazena os tipos dos parâmetros (se tiver), na quarta identifica se a função possui parâmetros, sendo True possui e False não, na quinta armazena o tipo da função e a na sexta um valor 0 e 1, sendo 1 função utilizada e 0 não utilizada.

Já para as variáveis, como a \texttt{golobal-b}, significa que ela possui escopo global, e a primeira posição do vertor armazena se é variável, a segunda o seu valor (\texttt{b}), a terceira se a variável é utilizada, a quarta se a variável foi inicializada (True sim e False não), a quinta armazena o tipo da variável e a sexta um parâmetro que não é alterado,
...
Em algumas dessas funções são realizadas a análise de erro, com o auxílio da tabela de símbolos. Para melhor compreensão, considere parte da função \texttt{var} demonstrada na Figura~\ref{var}.

\begin{figure}[h]
\caption{Análise semântica: Função var responsável por determinar erros.}
\label{var}
\begin{lstlisting}[frame=single] 
def var(self, node):
...
A função \texttt{var} é responsável por identificar e verificar se a variável (\texttt{node}) recebida por parâmetro foi declarada. Para isso é pesquisado na tabela de símbolos o escopo correspondente a variável, se é global ou se está em alguma função. Então verifica-se o tamanho desse nó filho podendo ser variável ou vetor, ou seja, pode ser \texttt{ID} ou \texttt{ID índice}. Se não for vetor, verifica-se se a variável está presente na tabela de símbolo. Caso não esteja, verifica-se ainda se ela foi declarada como escopo global, se isso não for verdade, quer dizer que a variável não foi declarada, mostrando uma mensagem de erro. Caso a variável esteja na tabela de símbolos, verifica-se se ela foi inicializada por meio da posição 3 na tabela, se for \texttt{False} é mostrado uma mensagem de erro que a variável não foi inicializada, caso contrário, atribui-se \texttt{True} na tabela de símbolos indicando que a variável foi declarada e armazena o seu tipo (inteiro ou flutuante). 
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{./asa}
